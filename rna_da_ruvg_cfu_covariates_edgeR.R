...
points(subset(top_rna$logCPM,top$PValue<=0.05 & top_rna$FDR <= 0.05 & (top_rna$logFC>=2|top_rna$logFC<=-2)),
       subset(top_rna$logFC,top$PValue<=0.05 & top_rna$FDR <= 0.05 & (top_rna$logFC>=2|top_rna$logFC<=-2)),
       pch=20,cex=0.5,col="red")
dev.off()

top_rna <- left_join(rownames_to_column(top_rna,var = "defined_taxa"),x_rna$genes,by="defined_taxa")
top_rna <- left_join(top_rna,a_logCPM,by="defined_taxa")
...
# rna_limma_daa_sig$Species[which(rna_limma_daa_sig$Species =="s__")] <- paste0("Unclassified|",sapply(1:length(which(rna_limma_daa_sig$Species =="s__")),FUN=function(y){
#   str_split(rna_limma_daa_sig$defined_taxa[which(rna_limma_daa_sig$Species =="s__")][y],"\\|")[[1]][2]}))
rna_limma_daa_sig$Genus[which(rna_limma_daa_sig$Genus =="g__")] <- paste0("Unclassified|",sapply(1:length(which(rna_limma_daa_sig$Genus =="g__")),FUN=function(y){
  str_split(rna_limma_daa_sig$defined_taxa[which(rna_limma_daa_sig$Genus =="g__")][y],"\\|")[[1]][2]}))
rna_limma_daa_sig <- rna_limma_daa_sig[order(rna_limma_daa_sig$logFC,decreasing = T),]

####Significant species/taxa heatmaps rna and rna####
mat_rna = as.matrix(rna_limma_daa_sig[,16:27])
mat_rna = apply(mat_rna, 2 ,as.numeric)
Sample = gsub("_\\d+_RNA", "", colnames(mat_rna))
# palette <- randomcoloR::distinctColorPalette(length(unique(c(rna_limma_daa_sig$Class,rna_limma_daa_sig$Class))))
# names(palette) <- as.character(unique(c(rna_limma_daa_sig$Class,rna_limma_daa_sig$Class)))
...
ht_rna = Heatmap(rna_limma_daa_sig$Class, name = "Class",width = unit(5, "mm"),
                 rect_gp = gpar(col = "white", lwd = 1),col = palette) +
  Heatmap(mat_rna, name = "log CPM", cluster_rows = T, 
          col=circlize::colorRamp2(c(range(mat_rna)[1], 0, range(mat_rna)[2]), c("#023FA5", "white", "#8E063B")),
  ...
  rowAnnotation(logFC = anno_barplot(rna_limma_daa_sig$logFC,baseline = 0, width = unit(1, "cm"))) +
  rowAnnotation(Pvalue = anno_barplot(rna_limma_daa_sig$PValue,baseline = 0, width = unit(1, "cm"))) +
  rowAnnotation(FDR = anno_barplot(rna_limma_daa_sig$FDR,baseline = 0, width = unit(1, "cm")))
cairo_pdf("rna_heatmap_logCPMs_logFC_class.pdf",width=7,height=3.5,family = "Times New Roman")
draw(ht_rna,merge_legend = TRUE, heatmap_legend_side = "right",annotation_legend_side = "right")
dev.off()

####Significant Salmonella barplot####
df_sal <- rna_limma_daa_sig[grep("Salmonella",rna_limma_daa_sig$Genus),]
m_df_sal <- reshape2::melt(df_sal,id.vars=c(colnames(df_sal)[1:15]))
m_df_sal$value <- as.numeric(m_df_sal$value)
m_df_sal$condition <- gsub("_\\d+_RNA","",m_df_sal$variable)
m_df_sal$replicate <-  gsub("_RNA","",m_df_sal$variable)
m_df_sal$replicate <-  gsub("Uninfected_|Infected_","",m_df_sal$replicate)
m_df_sal$replicate <- paste0("R",m_df_sal$replicate)
pc_rna <- ggplot(m_df_sal,aes(x=fct_relevel(condition,"Uninfected","Infected"),y=value)) + 
  ...
  annotate("text", x=1, y=2^3.5, label= paste0("P-value = ",round(unique(m_df_sal$PValue),4),"\nFDR = ",round(unique(m_df_sal$FDR),4))) + 
  ...
  ggtitle("Metatranscriptome") + scale_y_continuous(trans = scales::log2_trans(),
                                             breaks = scales::trans_breaks("log2", function(x) 2^x),
                                             labels = scales::trans_format("log2", scales::math_format(2^.x)))

cairo_pdf(filename = "rna_s__Salmonella_enterica_da_boxplot_log2CPM.pdf",width = 4.5,height = 2.5,
