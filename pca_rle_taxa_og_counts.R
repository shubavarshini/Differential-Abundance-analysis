...
####MG>=1####
output_dir <- "/home/shuba/Documents/microbiome/wildtype_MG_MT_nextflow/newFiles/salmon_GTDB_iMGMC_ihMAG_markerGenes/mockWT_GTDB_MGanyr1_noSalBongori_salmonQuant/qc_taxaOG_count"
...
count_sum$taxa_associated_OG <- paste0(count_sum$gtdb_taxa,"|",count_sum$associated_OG)

####MG>=40%####
# output_dir <- "/home/shuba/Documents/microbiome/wildtype_MG_MT_nextflow/newFiles/salmon_GTDB_iMGMC_ihMAG_markerGenes/mockWT_GTDB_iMGMC_ihMAG_MGperc40_salmonQuant/qc_taxaOG_count"
# if (!dir.exists(output_dir)) {dir.create(output_dir)}
# setwd(output_dir)
...
logcpm_dist_pca_rle <- function(degObject,outfile_prefix,folder){
  ...
  row.names(log_counts) <- degObject$genes$taxa_associated_OG
  log_counts_df <- rownames_to_column(data.frame(log_counts,stringsAsFactors = F),var = "taxa_associated_OG")
  m_log_counts_df <- reshape2::melt(log_counts_df,value.name = "log_CPM",id.vars=c("taxa_associated_OG"))
  ...
  medians <- log_counts_df %>%
    select(-taxa_associated_OG) %>%
    mutate(across(everything(), median)) 
  
  # Subtract the row-wise median from all columns except taxa_associated_OG
  ...
  m_rle_df <- reshape2::melt(rle_df,value.name = "rle",id.vars=c("taxa_associated_OG"))
...
ruvg_function <- function(dgeObject,normalize_method,outfile_prefix,folder){
  ...
  # empirical <- dgeObject$genes$taxa_associated_OG[which(!(dgeObject$genes$taxa_associated_OG %in% top$taxa_associated_OG[1:250]))]
  empirical <- top$taxa_associated_OG[which(top$PValue>0.8 & top$FDR>0.1)]
  ## ----emp_ruvg-----------------------------------------------------------------
  a <- floor(dgeObject$counts)
  row.names(a) <- dgeObject$genes$taxa_associated_OG
  ...
  for (i in 1:5) {
    ...
    normalisedCounts <- data.frame(cbind("taxa_associated_OG" =rownames(normCounts(set2)),edgeR::cpm.default(normCounts(set2),log=T)), stringsAsFactors = FALSE)
    m_normalisedCounts <- reshape2::melt(normalisedCounts,value.name = "normalised_count",id.vars=c("taxa_associated_OG"))
...
keep <- rowSums(count_sum[,8:31] != 0) >= 1
...
taxa <- as.matrix(count_removing_zeros[,c(1:7,32)],row.names=count_removing_zeros[,1])


data <- SummarizedExperiment(assays = list(counts=as.matrix(count_removing_zeros[,-c(1:7,32)],
                                                            row.names=count_removing_zeros[,c(1)])),
                             rowData = taxa,colData = DataFrame(Samples=colnames(count_removing_zeros)[-c(1:7,32)], 
                                                                Condition=gsub("_\\d+","",colnames(count_removing_zeros)[-c(1:7,32)]),
                                                                Type=str_split_fixed(colnames(count_removing_zeros)[-c(1:7,32)],"_",n=3)[,3],
                                                                Replicate=str_split_fixed(colnames(count_removing_zeros)[-c(1:7,32)],"_",n=3)[,2],
