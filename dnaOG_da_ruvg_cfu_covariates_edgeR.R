...
####MG>=1####
# output_dir <- "/home/shuba/Documents/microbiome/wildtype_MG_MT_nextflow/newFiles/salmon_GTDB_iMGMC_ihMAG_markerGenes/mockWT_GTDB_MGanyr1_noSalBongori_salmonQuant/qc_OG_taxa_count/ruvg_edgeR_cfu_covariates/MG_infectionCovariate"
...
####MG>=40%####
output_dir <- "/home/shuba/Documents/microbiome/wildtype_MG_MT_nextflow/newFiles/salmon_GTDB_iMGMC_ihMAG_markerGenes/mockWT_GTDB_iMGMC_ihMAG_MGperc40_salmonQuant/qc_OG_taxa_count/ruvg_edgeR_cfu_covariates/MG_infectionCovariate"
if (!dir.exists(output_dir)) {dir.create(output_dir,recursive = T)}
setwd(output_dir)
...
row.names(a) <- x_dna_ruvg$genes$associated_OG
...
corrected <- data.frame(cbind("associated_OG" =x_dna$genes$associated_OG,corrected), stringsAsFactors = FALSE)
...
y_dna <- DGEList(counts=counts(set2), group=x_dna$samples$group,genes = x_dna$genes)
y_dna <- calcNormFactors(y_dna, method=normalize_method)
...
top_dna <- left_join(top_dna,a_logCPM,by="associated_OG")
...
path_indices_forFry <- get_h_path_indices_forFry(h_path, y_dna$genes$associated_OG)

fry_dna <- fry(y_dna, path_indices_forFry, design=design_edgeR,
                   geneid=y_dna$genes$associated_OG,contrast=2)
...
f_dna <- reformat_h_path(fry_dna, h_path)
...
sig_f_dna <- significant_gene_list(f_dna,top_dna)
write.table(rownames_to_column(sig_f_dna,var = "GO"),file="dna_fry_results_withGO.tsv",row.names = F,quote = T,sep="\t")

cairo_pdf(file="rna_pvalue_fdr_fryGO.pdf",bg="transparent",
          width = 10,height = 2.5,family = "Times New Roman")
par(mfrow = c(1, 4),bg="transparent",family="Times New Roman")
hist(sig_f_dna$PValue,breaks=20,main = paste0("Histogram of P-value\nN=" , nrow(y_dna$counts)),
     xlab = "P-value")
hist(sig_f_dna$FDR,breaks=20,main = paste0("Histogram of FDR\nN=" , nrow(y_dna$counts)),
     xlab = "FDR")
hist(sig_f_dna$PValue.Mixed,breaks=20,main = paste0("Histogram of non-directional P-value\nN=" , nrow(y_dna$counts)),
     xlab = "P-value")
hist(sig_f_dna$FDR.Mixed,breaks=20,main = paste0("Histogram of non-directional FDR\nN=" , nrow(y_dna$counts)),
     xlab = "FDR")
dev.off()

sig_f_dna <- sig_f_dna %>% mutate(NGenes = ifelse(Direction == "Down", -NGenes, NGenes))
# Create the bar plot
pathway_bar_plot <- ggplot(subset(sig_f_dna,!is.na(sig_f_dna$path_name)), aes(x = NGenes, y = reorder(path_name, NGenes))) +
...
cairo_pdf("dna_all_GO_pathway_bar_plot.pdf",width=6,height=10,family = "Times New Roman",bg = "transparent")
print(pathway_bar_plot)
dev.off()

sig_pathways <- subset(sig_f_dna,sig_f_dna$PValue <= 0.05 & sig_f_dna$FDR <= 0.05 & 
                         sig_f_dna$NSignificantGenes > 0 & !is.na(sig_f_dna$path_name))
# Create the bar plot
...
cairo_pdf("dna_sig_GO_pathway_bar_plot.pdf",width=5,height=5,family = "Times New Roman",bg = "transparent")
...
if(length(which(f_dna$source == "KEGG_Pathway"))!=0){
  row_nos <- which(f_dna$source == "KEGG_Pathway")
  f_dna$path_name[row_nos] <- sapply(row_nos,function(x){
    tryCatch(KEGGREST::keggGet(rownames(f_dna)[x])[[1]]$NAME,error=function(e){NA})
  })
}
# ko00471="D-Glutamine and D-glutamate metabolism"

sig_f_dna <- significant_gene_list(f_dna,top_dna)
sig_f_dna$path_name[which(rownames(sig_f_dna)=="ko00471")] <- "D-Glutamine and D-glutamate metabolism"
write.table(rownames_to_column(sig_f_dna,var = "KEGG"),file="dna_fry_results_withKEGG.tsv",row.names = F,quote = T,sep="\t")
...
cairo_pdf("dna_all_KEGG_pathway_bar_plot.pdf",width=6,height=10,family = "Times New Roman",bg = "transparent")
print(pathway_bar_plot)
dev.off()
...
cairo_pdf("dna_sig_KEGG_pathway_bar_plot.pdf",width=6,height=6,family = "Times New Roman",bg = "transparent")
print(sig_pathway_bar_plot)
dev.off()
...
df <- top_dna %>% separate_rows(Class, sep = ";")
...
palette <- c("c__Clostridia" = "#DF7C5F","c__Bacteroidia" = "#DE77BD", "c__Verrucomicrobiae" = "#89E0A6", 
             ...
             "c__Myxococcia" = "#C5B1CF")

####Volcano plot####
dna_class <- ggplot(df, aes(x = logCPM, y = logFC,color=Class, fill=Class)) +
...
dna_pval_fdr <- ggplot(df, aes(x = logCPM, y = logFC, color = PValue, fill = FDR)) +
  ggrastr::geom_point_rast(size=1,alpha=0.2,raster.dpi = 150) +  
  scale_colour_gradient2(trans = "sqrt", name = "PValue",low = "brown", mid = "white", high = "grey", midpoint = .25) +
  scale_fill_gradient2(trans = "sqrt", name = "FDR",low = "brown", mid = "white", high = "grey", midpoint = .25) +
  theme_bw(base_size = 12,base_family = "Times New Roman") + theme(legend.position = "right") +
...
cairo_pdf(filename = "dna_og_ruvg_edgeR_MAplot.pdf",width = 5,height = 3.75,
